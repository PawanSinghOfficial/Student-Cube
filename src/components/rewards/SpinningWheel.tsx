...
const SEGMENTS = [
  { label: "7 Hours", hours: 7, color: "hsl(var(--primary))" },
  { label: "12 Hours", hours: 12, color: "hsl(var(--accent))" },
  { label: "18 Hours", hours: 18, color: "hsl(var(--success))" },
  { label: "1 Day", hours: 24, color: "hsl(var(--destructive))" },
];
...
export function SpinningWheel({ onResult }: SpinningWheelProps) {
  ...
  const drawWheel = (canvas: HTMLCanvasElement) => {
    ...
    SEGMENTS.forEach((seg, i) => {
      ...
      ctx.fillText(seg.label, radius * 0.6, 6);
      ctx.restore();
    });
  ...
  const spin = () => {
    ...
    // Spin 5-8 full rotations + land on winning segment
    ...
    <div className="flex flex-col items-center gap-6">
      <div className="relative">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 z-10">
          <div className="w-0 h-0 border-l-[12px] border-r-[12px] border-t-[24px] border-l-transparent border-r-transparent border-t-foreground" />
        </div>
